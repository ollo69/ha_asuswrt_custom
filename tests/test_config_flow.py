...
from custom_components.asuswrt_custom.const import (
    CONF_DNSMASQ,
    CONF_INTERFACE,
    CONF_REQUIRE_IP,
    CONF_SSH_KEY,
...
PATCH_GET_HOST = patch(
    "custom_components.asuswrt_custom.config_flow.socket.gethostbyname",
...
def patch_asuswrt(mac_addr=None, *, is_connected=True, side_effect=None):
    ...
    return {
        KEY_LEGACY: patch(
            "custom_components.asuswrt_custom.bridge.AsusWrtLegacy",
            return_value=AsusWrtLegacyFake(mac_addr, is_connected, side_effect),
        ),
        KEY_HTTP: patch(
            "custom_components.asuswrt_custom.bridge.AsusWrtHttp",
